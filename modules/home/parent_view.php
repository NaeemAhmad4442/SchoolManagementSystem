<?php
    $fetchData = myquery($db, "SELECT * FROM notifications WHERE reciever = (SELECT sonid FROM User WHERE ID = ".$_SESSION["userid"].") ORDER BY ID DESC LIMIT 10");
    $fetchData1 = myquery($db, "SELECT * FROM Note WHERE studentID = (SELECT sonid FROM User WHERE ID = ".$_SESSION["userid"].") ORDER BY ID DESC LIMIT 10");

    $result = myquery($db, "SELECT * FROM Test
    WHERE Test.classID = (SELECT User.classID FROM User WHERE User.ID = (SELECT sonid FROM User WHERE ID = ".$_SESSION["userid"].")) AND date > CURDATE() ORDER BY Test.ID DESC LIMIT 10");
    if(!empty($result))
    {
        echo '
